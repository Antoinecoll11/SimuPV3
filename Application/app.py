...
if "prix_injection" not in st.session_state:
    st.session_state["prix_injection"] = 0.05
...
def generer_profil_borne(date_series, puissance_borne_kw, horaires_borne, nb_jours_charge_semaine):
    ...
    for dt in pd.to_datetime(date_series):
        ...
        # On considère que la charge a lieu sur les nb_jours_charge_semaine premiers jours de la semaine
        actif_jour = jour_semaine < nb_jours_charge_semaine
...
if borne_active:
    ...
    nb_jours_charge_semaine = st.sidebar.slider(
        "Nombre de jours de charge par semaine",
        min_value=1,
        max_value=7,
        value=5,
        step=1
    ...
    energie_semaine_kwh = energie_jour_kwh * nb_jours_charge_semaine
    energie_an_kwh = energie_semaine_kwh * 52

    st.sidebar.caption(f"Charge par jour : {energie_jour_kwh:.1f} kWh")
    st.sidebar.caption(f"Charge par semaine : {energie_semaine_kwh:.1f} kWh")
    st.sidebar.success(f"Consommation annuelle estimée : {energie_an_kwh:,.0f} kWh".replace(",", " "))
...
if borne_active:
    mon_tableau["Conso_Borne"] = generer_profil_borne(
        ...
        nb_jours_charge_semaine=nb_jours_charge_semaine
    )
else:
...
with tab_finance:
    ...
    # Recherche de la meilleure puissance
    ...
    # Affichage des métriques de la meilleure option
    c1, c2, c3, c4 = st.columns(4)

    c1.metric(
        "Puissance optimale",
        f"{meilleure_puissance} kW"
    )

    c2.metric(
        "Redevance fixe",
        f"{meilleure_ligne['Redevance fixe annuelle (€)']:.2f} €"
    )

    c3.metric(
        "Coût dépassement",
        f"{meilleure_ligne['Coût du dépassement (€)']:.2f} €"
    )
    ...
    # Mise en évidence de la meilleure option
    ax_reseau.scatter(
        meilleure_puissance,
        meilleur_cout,
        s=100,
